...
*Setup instructions for a Gitian build of vRolexCoin using a Debian VM or physical system.*
...
- [Building vRolexCoin](#building-vrolexcoin)
...
Clone the git repositories for vrolexcoin and Gitian and then checkout the vrolexcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the vrolexcoin repository
...
To build vRolexCoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the vrolexcoin repository.
...
For example:
```bash
URL=https://github.com/user/vrolexcoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/vrolexcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/vrolexcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/vrolexcoin-build.assert
```
...
[vrolexcoin/gitian.sigs](https://github.com/vrolexcoin-project/gitian.sigs.VRC/) repository, or if that's not possible create a pull
request. 
